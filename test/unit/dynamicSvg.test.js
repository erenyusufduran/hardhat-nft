  ...
  : describe("Dynamic SVG NFT Unit Tests", function () {
      ...
      describe("mintNft", () => {
        ...
        it("shifts the token uri to lower when the price doesn't surpass the highvalue", async function () {
          const highValue = ethers.utils.parseUnits("6", "ether");
          ...
          const updateAnswerTx = await mockV3Aggregator.updateAnswer(lowValue.toString());
          updateAnswerTx.wait(1);
          const updatedContractTokenURI = await dynamicSvgNft.tokenURI("0");
